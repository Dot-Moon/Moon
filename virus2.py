...
folder = tkFileDialog.askdirectory() ##GUI폴더설정
...
def searchvirustotal(dirname):
    ...
    for filename in filenames:
        ...
        response = requests.get('https://www.virustotal.com/vtapi/v2/file/report',   ##받는부분
        ...
        for h in json_response['scans']: ##시트에 입력하는 부분
            ...
            i=i+1
    workbook.save(str(strftime("%y-%m-%d_%H(h)_%M(m)_%S(s).xls",localtime()))) ##저장

searchvirustotal(folder)
